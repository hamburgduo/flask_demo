...
class MyCompany(UserBase):
    ...
    def enabled_jobs(self):
        return self.all_time_job.filter(AllTimeJob.is_enable.is_(True))

    def enabled_part_time_jobs(self):
        return self.part_time_job.filter(PartTimeJob.is_enable.is_(True))


class AllTimeJob(Base):
    __tablename__ = "all_time_job"
    ...
    company = db.relationship(
        "MyCompany", uselist=False, backref=db.backref("all_time_job", lazy="dynamic")
    )
    # 职位描述
...
class PartTimeJob(Base):
    ...
    company_id = db.Column(db.Integer, db.ForeignKey("company.id", ondelete="CASCADE"))
    company = db.relationship(
        "MyCompany", uselist=False, backref=db.backref("part_time_job", lazy="dynamic")
    )
    # 职位描述
    description = db.Column(db.Text)
    # 职位待遇
...
class Delivery(Base):
    ...
    job_id = db.Column(
        db.Integer, db.ForeignKey("all_time_job.id", ondelete="SET NULL")
    )
    job = db.relationship(
        "AllTimeJob", uselist=False, backref=db.backref("delivery", lazy="dynamic")
    )
